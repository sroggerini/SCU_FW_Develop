...
extern osThreadId_t ethInitTaskHandle;
extern const osThreadAttr_t ethInitTask_attributes;
...
static void     funzioniBoardRepair       (uint8_t* msgRcv);
    ...
    case STATE_UI_PASSWORD:
      if (telnetMsg.telnetEv == TELNET_RX)
      {
        if (strstr((char*)telnetMsg.bufferMsg, "\r\n") != NULL)
        {
          ...
          if (numLoginError >= 3)
          {
            /* to mitigate brute force attack */
            suspendEthPollingCheck(ETH_EVENT_SUSPEND_TIME);    // stop the ethernet physical for a little time 
            numLoginError = 0;
          }
        }
        else
...
static void consoleMng (uint8_t* buff )
{
  ...
  switch (console.stato)
  {
   ...
   case COMANDO_SET_USER_PIN:
    ...
    case COMANDI_BOARD_REPAIR:
      funzioniBoardRepair (buff);
      break;
...
static void idleConsole (uint8_t* msgRcv)
{
  ...
  switch (msgRcv[0])
  {
    ...
    case SEL_INFO_REPAIR_CMD:               // digitato '3'
      if (isSemMasterFz() == TRUE)
      {
        console.stato = COMANDI_BOARD_REPAIR;
      }
      PrintMenu ();
      break;

    case SEL_EE_CMD:                       // digitato '4'
      console.stato = COMANDI_EEPROM;
      PrintMenu ();
      break;

    case SEL_QSPI_CMD:                       // digitato '5'
      console.stato = COMANDI_FLASH;
      PrintMenu ();
      break;

    case SEL_STATION_CMD:                    // digitato '6'
      console.stato = COMANDI_STATION_MNG;
      PrintMenu ();
      break;

    case SEL_EM_CMD:                        // digitato '7'
      console.stato = COMANDI_EM_MNG;
      PrintMenu ();
      break;

    case SEL_MODBUS_CMD:                   // digitato '7'
      console.stato = COMANDI_MODBUS;
      PrintMenu ();
      break;

    case SEL_IP4_CMD:                       // digitato '9'
...
//  unsigned char*    filename; 
  ...
  switch (console.stato)
  {
    case CONSOLE_IDLE:
      ...
      tPrintf (" \r\n\r\n 1. Time and Date\r\n 2. LCD test\r\n 3. Board Repair (only for master SEM)\r\n 4. EEPROM test\r\n 5. FLASH QSPI\r\n");      
      tPrintf (" 6. Station Mng\r\n 7. Energy Meter\r\n 8. Modbus command\r\n 9. IP config\r\n");
    ...
    case COMANDO_DATA_ORA:
      // Aggiorno data e ora
      (void)getCurrentLocalTime((uint32_t*)&temp);
      tPrintf("Current date and Time %s\n\r", (char*)temp);
   ...
   case COMANDO_SHOW_ACT_KEY:
    {
      ...
      memcpy(key, "xxxxx\0", 6);
      SecureArea_getActivationKey(key, 6);
      tPrintf("Current activation key %.*s\r\n", 5, key);
      tPrintf("m. Return\r\n");
    }
    break;
   ...
   case COMANDO_SET_USER_PIN:
    ...
    case COMANDI_FLASH:
      ...
      tPrintf(" t - Erase all transactions \n\r p x - set protection level x [0,1,2]\n\r");
      tPrintf(" d x - downgrade / no reset SBC [0,1] 1=permesso / SBC reset suspended \n\r m. Main menu\n\r");
      m = __iar_dlmallinfo();
    ...
    case COMANDI_BOARD_REPAIR:
      tPrintf("[Board Repair]\n\r");
      tPrintf(" i. info repair status\n\r");
      tPrintf(" r. Reset all info repair info\n\r");
      tPrintf(" m. Main menu\n\r");
      break;
    ...
    case COMANDI_EEPROM:
      ...
      tPrintf(" R Erase all eeprom data and secure area \n\r");
      tPrintf(" B broadcast for all SCU factory restore & Secure Area\n\r");
...
#if defined HW_MP28947 && defined EXCLUDE_ETHERNET       
      tPrintf(" e x - 0 = Ethernet clock disabled - 1 = Ethernet clock enabled\n\r");
#endif      
...
static void funzioniQSPIflash (uint8_t* msgRcv)
{
  ...
  switch (msgRcv[0])
  {
    ...
    case 'p':
      if  ((msgRcv[2] == '1')|| (msgRcv[2] == '2'))
      {
        RetVal = (msgRcv[2] == '1') ? OB_RDP_LEVEL_1 : OB_RDP_LEVEL_2;
        setRplOptionByte(RetVal); 
      }
      break;

    case 'd':
      if  ((msgRcv[2] == '1') || (msgRcv[2] == '0'))
      {
        RetVal = (msgRcv[2] == '1') ? 0xAB : 0;
        setDowngradeStatus(RetVal); 
        if  (msgRcv[2] == '1')
          tPrintf("Downgraded  Allowed!!\n\r");
        else
          tPrintf("Downgraded Not Allowed!!\n\r");
      }
      break;
      ...
      if ((msgRcv[1] == ' ') && (msgRcv[22] == '\r') && (msgRcv[23] == '\n'))
      {
        if ((msgRcv[2] == '2') && (msgRcv[3] == '0'))
        {
          ...
          if (msgRcv[10] == ' ')
          {
            ...
            /* save DST flag   *****/
            EEPROM_Save_Config (DST_EADD, (uint8_t*)&locDateTime.dstFlag, 1);
            ...
            /* save time zone   *****/
            EEPROM_Save_Config (TIME_ZONE_EADD, (uint8_t*)&timeZone, 1);
...
static void funzioneSetProductSn (uint8_t* msgRcv)
{
  ...
  switch (msgRcv[0])
  {
    case 'r':
      ...
      EEPROM_Save_Config (PRD_SN_EE_ADDRES, PrdSn, 5);
      tPrintf("All 'FF' to SN!!\n\r");
      break;
...
static void funzioneSetHts (uint8_t* msgRcv)
{
  ...
  switch (msgRcv[0])
  {
    ...
    case '1':
      ...
      {
        msgRcv[2] -= '0';
        EEPROM_Save_Config (TEMP_CTRL_ENB_EADD, &msgRcv[2], 1);
    ...
    case '2':
      if ((msgRcv[1] == ' ') && (isdigit(msgRcv[2]) != 0) && ((isdigit(msgRcv[3]) != 0) || (msgRcv[3] == 0x0D)))
      {
        ...
        EEPROM_Save_Config (TEMP_CTRL_VAL_EADD, &dData, 1);
    ...
    case '3':
       ...
       || ((msgRcv[1] == ' ')                       && (((isdigit(msgRcv[2]) != 0) || (isdigit(msgRcv[3]) != 0)) || ((isdigit(msgRcv[2]) != 0) || (msgRcv[3] == 0x0D)))))
      {
        ...
        EEPROM_Save_Config (TEMP_DELTA_EADD, &dData, 1);
    ...
    case '4':
      if ((msgRcv[1] == ' ') && (isdigit(msgRcv[2]) != 0) && ((isdigit(msgRcv[3]) != 0) || (msgRcv[3] == 0x0D)))
      {
        ...
        EEPROM_Save_Config (TEMP_HYSTERESIS_EADD, &dData, 1);
...
static void funzioneSetUserPin (uint8_t* msgRcv)
{
  ...
  {
    case 'r':
      esito = SecureArea_storeUserPin("00000", 5);
      if(esito == 0)
        tPrintf("All '0' to user pin!!\n\r");
      else
        tPrintf("Error during reset pin\n\r");
      break;
    ...
    case 'u':
      if (msgRcv[1] == ' ')
      {
        ...
        if (esito)
        {
          ...
          userPin[1] = msgRcv[3];
          userPin[0] = msgRcv[2];
          SecureArea_storeUserPin(( char* )userPin, 5);
  ...
  switch (msgRcv[0])
  {
      ...
      else
      {
        switch (msgRcv[2])          
        {
          case '0':
            ...
            tPrintf("Ethernet clock disabled!\n\r");            
            break;
          case '1':
            Eth_send_Activate_Event();
            tPrintf("Ethernet clock enabled!\n\r");            
            break;
        }
      }
      break;
    ...
    case 'Q': // o 0 or t 1..9 
      ...
      else
      {
        ...
        EEPROM_Save_Config (POST_SUSP_TIME_EADD, (uint8_t *)&edata, 1);
        if (msgRcv[2] > '0')
        ...
        else
        {
          ...
          else
          {
            ...
            if (hexAdd[0] != add)
            {
              ...
              EEPROM_Save_Config (RS485_ADD_EADD, (uint8_t*)&hexAdd[0], 1);
...
static void funzioniBoardRepair (uint8_t* msgRcv)
{
  uint16_t          cnt, mask, status;
  uint8_t           charP;
  uint8_t           stringPrint[50];
  ...
  switch (msgRcv[0])
  {
    ...
    case 'i':
      status = getInfoEepromSlaveStatus();
      tPrintf("02 03 04 05 06 07 08 09 10 11 12 13 14 15 16\n\r");
      memset((void*)stringPrint, 0x20, sizeof(stringPrint));
      for (cnt = 1, mask = 0x02; cnt < SCU_NUM; cnt++, mask = mask << 1)
      {
        if ((status & mask) != 0) charP = '-'; else charP = 'X';
        stringPrint[(cnt-1)*3] = charP;
      }
      stringPrint[(cnt-1)*3 + 1] = '\0';
      tPrintf("%s\n\r", stringPrint);
      break;
...
/**
*
* @brief        Erase all eeprom info for all slave SCU
*
* @param [in]   none
*
* @retval       none
...
void eraseAllBoardInfoEeprom (void)
{
  ...
  if (pConfPar != NULL)
  {
    ...
    if (ferr == FR_OK)
    {
      tPrintf("All board info for repair cancelled!\n\r");
    }
  }
}
