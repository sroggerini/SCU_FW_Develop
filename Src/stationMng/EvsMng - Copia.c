...
static uint8_t              iso15118_info_test;
static iso15118_state_en    evs_iso15118_state;


static uint8_t v2g_array[16];
static uint8_t v2g_idx;
static uint8_t IdReq_array[16];
static uint8_t IdReg_idx;
static uint8_t term_array[16];
static uint8_t term_idx;
static uint8_t pause_array[16];
static uint8_t pause_idx;
...
if ((pMsg->EvsMngEvent == EVS_EXTERNAL_EM_GOOD) || (pMsg->EvsMngEvent == EVS_SINAPSI_CHN2_GOOD))
    ...
    if (pMsg->EvsMngEvent == EVS_EXTERNAL_EM_GOOD)
        ...
        if (sinapsi_inst_status != 1)
            ...
            eeprom_param_set(SINAPSI_INST_EADD, &sinapsi_inst_status, 1);
            }
...
if ((control_enable & EMETER_EXT_CRL2) && ((control_enable & SINAPSI_CHN2_CRL2) == 0))
    ...
    eeprom_param_set(CONTROL_BYTE2_EADD, &control_enable, 1);
    }
...
uint32_t    rtc_reg_save;
        ...
        if (pMsg->EvsMngEvent == EVS_ISO15118_INFO_DEVICE_UPDATE)
            {
            v2g_array[v2g_idx] = ISO15118_Info_Device.State_openv2g;
            v2g_idx ++;
            v2g_idx &= 0x0F;
            
            IdReq_array[IdReg_idx] = ISO15118_Info_Device.serviceIdReq;
            IdReg_idx ++;
            IdReg_idx &= 0x0F;

            term_array[term_idx] = info.device.terminateOpenV2G;
            term_idx ++;
            term_idx &= 0x0F;

            pause_array[pause_idx] = info.device.pauseOpenV2G;
            pause_idx ++;
            pause_idx &= 0x0F;
            }
...
switch (evs_state)
        ...
        if (pMsg->EvsMngEvent == EVS_ISO15118_INFO_DEVICE_UPDATE)
            ...
            switch (evs_iso15118_data_req)
                ...
                case ISO15118_CHARGING_PAUSE:
                    iso15118_info_test = ISO15118_Info_Device.serviceIdReq;
                    break;
                case ISO15118_V2GSEQUENCEEND:
                    iso15118_info_test = ISO15118_Info_Device.State_openv2g;
                    break;
                }

            if (iso15118_info_test == evs_iso15118_data_req)
                {
    ...
    case EVSTATE_S2_WAITING:  // wait for S2 closing
        ...
        else if ((pMsg->EvsMngEvent == EVS_S2_STATE_UPDATE) || (evs_state_resume == EVSTATE_M3S_M3T_DETECT))
            {
            if ((plug_state_get() == PLUG_INSERTED) || (evs_state_resume == EVSTATE_M3S_M3T_DETECT))
                {
                if ((s2_state_get() == S2_STATE_D) && (getInput(IN7_VENT_EXP0) == GPIO_PIN_SET) && (control == VENT_CRL0))
                    {
                    evs_error_set(CONTROL_BYTE_0, VENT_ANOM0, 1);
