...
*
* @vcsInfo
*     $Id: ExtFlash.c 76 2022-06-20 09:46:05Z npiergi $
*
*     $Revision: 76 $
*
*     $Author: npiergi $
...
#ifdef SOSPESA
static uint8_t BSP_QSPI_setNonVolatileReg (uint8_t *pVal);
#endif
...
static uint8_t BSP_QSPI_SetQuadMode(void)
{
  ...
  if ((QSPIModel == QSPI_W25Q80DV) || (QSPIModel == QSPI_W25Q016DV))
  {
  ...
  else if ((QSPIModel == QSPI_IS25LP016) || (QSPIModel == QSPI_IS25LP080))
  {
    QSPI_CommandTypeDef     s_command;
    QSPI_AutoPollingTypeDef s_config;
  ...
  else if (QSPIModel == QSPI_SST26VF016B)
  {
    QSPI_CommandTypeDef     s_command;
    QSPI_AutoPollingTypeDef s_config;
...
#ifdef SOSPESA
/**
  * @brief  Write non-volatile-configuration-reg in MT25QL01G QSPI memory.
  * @retval QSPI memory status
  */
static uint8_t BSP_QSPI_setNonVolatileReg(uint8_t *pVal)
{
  ...
  if ((QSPIModel == QSPI_MT25QL01G) || (QSPIModel == QSPI_MT25Q512A))
  {
    /* Initialize non-volatile-configuration register  */
    s_command.InstructionMode   = QSPI_INSTRUCTION_1_LINE;
    s_command.Instruction       = WRITE_NONVOL_CFG_REG_CMD;
    s_command.AddressMode       = QSPI_ADDRESS_NONE;
    s_command.AlternateByteMode = QSPI_ALTERNATE_BYTES_NONE;
    s_command.DataMode          = QSPI_DATA_1_LINE;
    s_command.DummyCycles       = 0;
    ...
    /* Transmission of the data */
    if (HAL_QSPI_Transmit(&hqspi, pVal, HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
    {
      return QSPI_ERROR;
    }
...
void FlashInit(void)
{
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          QSPIModel = QSPI_IS25LP016;
          /* QUADSPI parameter configuration: number of address bit  */
          hqspi.Init.FlashSize = FLASH_016_ADDRESS_SIZE;
        }
        else
        {
          if (memcmp(FlashID, "\xBF\x26\x41", 3) == 0)    /* "\xBF\x26\x41"  QSPI_SST26VF016B Microchip - 26V - 16Mb */
          {
            QSPIModel = QSPI_SST26VF016B;
            /* QUADSPI parameter configuration: number of address bit  */
            hqspi.Init.FlashSize = FLASH_016_ADDRESS_SIZE;
          }
          else
          {
            QSPIModel = QSPI_UNKNOWN;
          }
        }
      }
    }
  ...
  if (QSPIModel != QSPI_UNKNOWN)
  {
    ...
    if (BSP_QSPI_setNonVolatileReg(reg) != HAL_OK)
    {
      // Flash in errore, blocco tutto
      while(1);
    }

    // Assegno gli indirizzi sempre fissi, quello della FAT...
    AddressFAT = FAT_BLK * FlashDimBlk;
    // ...quello della configuratione...
    AddressConfiguration = AddressFAT + ((sizeof(FAT) / FlashDimBlk) + 1) * FlashDimBlk;
    // ...e l'inizio degli archivi
    AddressArchive = AddressConfiguration + ((MAX_CONFIG_FILE_SIZE / FlashDimBlk) + 1) * FlashDimBlk;
#endif
  }
  else
  {
...
/**
  ...
  */
uint8_t BSP_QSPI_Detected(void)
{
  /* Configure the structure with the memory configuration */
  if ((QSPIModel == QSPI_W25Q80DV) || (QSPIModel == QSPI_W25Q016DV) ||
      (QSPIModel == QSPI_IS25LP016) || (QSPIModel == QSPI_SST26VF016B) || (QSPIModel == QSPI_IS25LP080))
  {
    return QSPI_OK;
  }
  else
...
/**
  ...
  * @retval QSPI memory status
  */
uint8_t raw_BSP_QSPI_Erase_SubBlock(uint32_t BlockAddress)
{
...
/**
  ...
  * @retval QSPI memory status
  */
uint8_t raw_BSP_QSPI_Write(uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
  refreshWD();
...
/**
  ...
  * @retval QSPI memory status
  */
uint8_t raw_BSP_QSPI_Read(uint8_t* pData, uint32_t ReadAddr, uint32_t Size)
{
...
/**
  ...
  * @retval QSPI memory status
  */
uint8_t raw_BSP_QSPI_EraseDataArea(void)
{
  uint32_t      qspiAdd;
  uint8_t       result;
  ...
  if(osSemaphoreAcquire(semaphoreFat, portMAX_DELAY) == osOK)
  {
    ...
    {
      result = raw_BSP_QSPI_Erase_SubBlock(qspiAdd);
      refreshWD();
