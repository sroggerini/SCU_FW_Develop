...
#ifdef HW_MP28947
#define     TIMER_TICK_POLL_REGS          ((uint16_t)1000)
#else
...
uint16_t              EM_INT_SCAME_FWver;
uint16_t              EM_EXT_SCAME_FWver;
            ...
            if (error == EM_NO_ERROR)
            {
              ...
              switch (emMsg.emModelInfo[ix].type)
              {
                ...
                case EM_PA775:     /* Found Energy meter plugged with board MP28947 */
                  ...
                  if (emMsg.emModelInfo[INTERNAL_EM].emPhase == EM_MONO_PH)
                  {
                     ...
                     setStationEmType(MONO_PA775, (emEnum_e)ix, EMETER_MONO_PH_ALGO2, EMETER_MONO_PH_PA775);
                  }
                  else   /* device THREE phase  */
                  {
                     ...
                     setStationEmType(TRI_PA775, (emEnum_e)ix, EMETER_THREE_PH_ALGO2, EMETER_THREE_PH_PA775);
                  }
                ...
                switch (emMsg.emModelInfo[ix].type)
                {
                  ...
                  case EM_LOVATO:
                    ...
                    if (error == EM_NO_ERROR)
                    {
                      ...
                      SCU_Param_Set ((uint8_t *)&SCU_param.TotalEnergy, (uint8_t *)&pass[0], 4);   
                    }
                    break;
                    
                  case EM_SCAME:
                    /* Get FW version */
                    getEmRegister(EC_FW_VERS_MAJOR_REG, 1, &pass[0], (uint8_t)EM_INT_ADDR, (uint8_t)FALSE);
                    getEmRegister(EC_FW_VERS_MINOR_SUB_REG, 1, &pass[2], (uint8_t)EM_INT_ADDR, (uint8_t)FALSE);
                    EM_INT_SCAME_FWver = pass[0] << 12 | pass[1] << 8 | pass[2] << 4 | pass[3];
                    break;
              ...
              else
              {
                switch (emMsg.emModelInfo[ix].type)
                {
                  case EM_SCAME:
                    /* Get FW version */
                    getEmRegister(EC_FW_VERS_MAJOR_REG, 1, &pass[0], (uint8_t)EM_EXT_ADDR, (uint8_t)FALSE);
                    getEmRegister(EC_FW_VERS_MINOR_SUB_REG, 1, &pass[2], (uint8_t)EM_EXT_ADDR, (uint8_t)FALSE);
                    EM_EXT_SCAME_FWver = pass[0] << 12 | pass[1] << 8 | pass[2] << 4 | pass[3];
                    break;
                  default:
                    break;
                }
      ...
      case EM_POLLING:
        ...
        if (pEmMsg->taskEvent == EVT_TIMEOUT)   
        {
          ...
          if (ix == INTERNAL_EM)
          {
            ...
            /* Check if measure are in range */
            if (METRO_CheckStatus() == STATUS_STPM_LINK_READ_ERROR)
            {
              METRO_Init ();
              METRO_Init_p2 ();
            }
              ...
              else
              {
                    ...
                    ((emMsg.emModelInfo[ix].modelType == 0x0009) && (emMsg.emModelInfo[ix].type == EM_IOM2G)))
                {
                  ...
                  if (lenInfo != regAddr)
                  {
                    ...
                    lenInfo /= 100;
                    SCU_Param_Set ((uint8_t *)&SCU_param.Pmng.Power, (uint8_t*)&lenInfo, 2);  /* ex PMNG_PWRLSB_EADD */
                  }
...
uint8_t get_emeter_type(emEnum_e emNum)
{
  if (emNum < EM_MAX_NUM)
  {
    if ((INTERNAL_EM == emNum) && ((emeter_type[(uint8_t)emNum] == EMETER_THREE_PH_LOVATO) ||  (emeter_type[(uint8_t)emNum] == EMETER_MONO_PH_LOVATO) ||
        ...
        (emeter_type[(uint8_t)emNum] == EMETER_THREE_PH_PA775) || (emeter_type[(uint8_t)emNum] == EMETER_MONO_PH_PA775)))  
    {
      if ((emeter_type[(uint8_t)emNum] == EMETER_THREE_PH_LOVATO) || (emeter_type[(uint8_t)emNum] == EMETER_THREE_PH_SCAME) || 
          (emeter_type[(uint8_t)emNum] == EMETER_THREE_PH_PA775))
      {
...
emModelInfo_st* discoveryEm (uint8_t emId)
{
  ...
  volatile      emError_e     emError;
  emEnum_e      emIx;
  volatile      uint8_t       len, ix, jx;
...
{
  return (SCU_param.TotalEnergy * 100); 
}
...
{
  return(pEmSinapsiInfo->potAttPreIstTs); /* potAttImmIstTs si riferisce alla potenza immessa Ha senso solo per contatori ENEL bidirezionali */
                                          /* potAttImmIstTs nei contatori ENEL unidirezionali vale sempre 0 quindi non si può usare "Caso Milani" */ 
}
