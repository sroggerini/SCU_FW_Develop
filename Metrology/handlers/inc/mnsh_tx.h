...
* MACROS & CONSTANTS:
*******************************************************************************/
#define APP_RX_DATA_SIZE        (8 * 1024)
...
#ifdef USE_USB     
#define MNSH_BUF_SIZE           (8 * 1024) 
#else
#define MNSH_BUF_SIZE           (1 * 512) 
#endif
#define MNSH_TIMER              (100)
...
typedef struct
{
  ...
  uint8_t       mnshTimer;
  msg_t         msg;  
  uint8_t       *mnshPrintfReadPtr;
  uint8_t       *mnshPrintfWritePtr;
  uint8_t       *mnshScanReadPtr;
  uint8_t       *mnshScanWritePtr;
  uint16_t      usbReceiveReadcnt;
  uint16_t      usboutcount;
...
* GLOBAL VARIABLES:
*******************************************************************************/
extern uint8_t mnshPrintfBuf[SHELL_PRINTF_BUF_SIZE];

extern mnshVars_t mnshVars;
extern uint8_t mnshTxBuf[MNSH_BUF_SIZE];
extern uint8_t mnshRxBuf[MNSH_BUF_SIZE];
...
void MNSH_UsbRxHandler_VCP(void);
void MNSH_UsbTxHandler_VCP(void);

uint8_t** getRxInPtrBuffer(void);
